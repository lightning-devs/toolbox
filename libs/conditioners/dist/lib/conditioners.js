 ...
 */
const getConditionalPredicate = (conditioners, functionSeeker) => {
    if (lodash_1.isEmpty(conditioners))
        throw new Error('The "when" clause must have at least 1 conditioner.');
    const conditionersToCompose = conditioners.map((conditioner) => {
        const conditionerKeys = Object.keys(conditioner);
        if (conditionerKeys.length > 2)
            throw new Error('There to many properties on the conditional case');
        if (conditionerKeys.length === 0)
            throw new Error('Some conditioners don\'t have the right amount of parameters');
        ...
        const functionToApply = functionSeeker(value) || (() => val => val);
        const partialFunction = lodash_1.isEmpty(params) ? functionToApply : functionToApply(...params);
        ...
        return (currentParameters) => {
            ...
            if (!currentBoolean && isAndRelation)
                return currentParameters;
            const gottenBoolean = partialFunction(currentValue);
            if (typeof gottenBoolean !== 'boolean')
                return currentParameters;
            const actualBoolean = shouldNegateBoolean ? !gottenBoolean : gottenBoolean;
            let nextBoolean = actualBoolean;
            if (isOrRelation)
                nextBoolean = currentBoolean || actualBoolean;
            if (isAndRelation)
                nextBoolean = currentBoolean && actualBoolean;
            return { currentBoolean: nextBoolean, currentValue };
        };
    }, []);
    return lodash_1.flow((currentValue) => ({ currentBoolean: false, currentValue }), ...conditionersToCompose);
};
/**
 ...
 */
exports.getConditioner = (...sourceFunctions) => (conditionalSequence) => {
    const { cases: conditionalCases = [] } = conditionalSequence;
    const functionSeeker = function_seeker_1.getFunctionSeeker(sourceFunctions);
    return (initialValues) => {
        ...
        if (lodash_1.isEmpty(successCase))
            return null;
        if (lodash_1.isEmpty(successCase.returns))
            throw new Error('Every conditional case must have a "returns" property.');
        return {
            initialValues,
...
//# sourceMappingURL=conditioners.js.map